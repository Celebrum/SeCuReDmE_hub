...
class RemoteKernelManager(MappingKernelManager):
    ...
    def start_kernel(self, kernel_id=None, path=None, **kwargs):
        ...
        if kernel_id is None:
            myfile = ""
            myur = ""
            with open('/tmp/access.log') as accessLog:
                for line in accessLog:
                    if "ipy" in str(line) and "GET" in str(line):
                        for elem in line.split():
                            if "ipy" in str(elem):
                                for subelem in elem.split("/"):
                                    if "ipy" in str(subelem):
                                        os.environ["KERNEL_NOTEBOOK_NAME"] = subelem.split(".")[0]
            open('/tmp/access.log', 'w').close()
            kernel_name = kwargs.get('kernel_name', 'python3')
            self.log.info("Request new kernel at: %s" % self.kernels_endpoint)
            os.environ["KERNEL_START_TIME"] = datetime.now().strftime("%Y%m%d%s")
